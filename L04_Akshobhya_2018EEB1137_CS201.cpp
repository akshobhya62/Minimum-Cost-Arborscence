...
void DFS_(graph g,ll s,ll**a,ll**old,ll*distance,ll*pred)
{
    ...
    while(head!=NULL&&g.adj[s-1].lis.Size()!=0)
    {
        ...
        {
            cout<<head->data<<"'s predecessor is "<<pred[head->data-1]<<endl;
            if(pred[head->data-1] == s)
                {
                    distance[head->data-1] = distance[s-1] + a[s-1][head->data-1];
                    cout<<"distance of "<<head->data<<" updated to "<<distance[s-1] + a[s-1][head->data-1]<<endl;
                }
    ...
    while(t--)
    {
        ...
        for(ll i=0;i<g.vertices;i++)
        {
                if(modified[i])
                {
                    ...
                    while(ptr!=NULL&&g.adj[i].lis.Size()!=0)
                    {
                        G.addEdge(g.adj[valueToBeStored-1].data, g.adj[convert[ptr->data-1]-1].data);
                        weight[g.adj[valueToBeStored-1].data-1][convert[g.adj[ptr->data-1].data-1]-1] = aa[g.adj[i].data-1][g.adj[ptr->data-1].data-1];
                        ptr = ptr->next;
                    }
                    if(g.adj[i].lis.Size()!=0)
                    valueToBeStored++;
